...
# 其中<x> = (min(x1, x2, x3, x4) + max(x1, x2, x3, x4)) / 2
# <y> = (min(y1, y2, y3, y4) + max(y1, y2, y3, y4)) / 2
            ...
            for line in lines:
                ...
                if line:
                    try:
                        ...
                        error_file_count += 1
                        continue
                    x = (min(float(x1), float(x2), float(x3), float(x4)) + max(float(x1), float(x2), float(x3), float(x4))) / 2
                    y = (min(float(y1), float(y2), float(y3), float(y4)) + max(float(y1), float(y2), float(y3), float(y4))) / 2
                    width = max(float(x1), float(x2), float(x3), float(x4)) - min(float(x1), float(x2), float(x3), float(x4)) + diff * 2
                    height = max(float(y1), float(y2), float(y3), float(y4)) - min(float(y1), float(y2), float(y3), float(y4)) + diff * 2
                    if x + width/2 > 1:
                        width = (1 - x)/2
                    if y + height/2 > 1:
                        height = (1 - y)/2
